...
void GenIR::visit(StmtAST &ast) {
    switch (ast.sType) {
        ...
        case ASS: {
            // ******************* 代码填写处
            //左值：通知LVal节点本次需要的是地址(指针)而非load出的值
            requireLVal = true;
            ast.lVal->accept(*this);
            Value *lValPtr = recentVal;     //左值地址，类型为 i32* 或 float*

            //右值：求出待赋的表达式的值
            ast.exp->accept(*this);
            Value *rVal = recentVal;        //右值，类型为 i32 / float / i1

            //左值指针所指向的类型
            Type *pointee = static_cast<PointerType *>(lValPtr->type_)->contained_;

            //比较结果(i1)先扩展为i32，再与左值类型对齐
            if (rVal->type_ == INT1_T) {
                rVal = builder->create_zext(rVal, INT32_T);
            }
            //LLVM IR是强类型，store的val必须与指针所指类型一致，必要时做类型转换
            ...
            //生成store指令：将右值存入左值地址(注意参数顺序: val, ptr)
            builder->create_store(rVal, lValPtr);
            // ******************* 代码结束
            break;
        }
        case EXP:
